...
import { SaveEventMutation, useSaveEventMutation } from "../../../generated/graphql";
...
function AddEvent() {
  ...
  return (
    <Fragment>
      {showModal && <LoginContainer view={"Login"} onClose={() => setShowModal(false)} />}
      <CustomAlert auth={auth} data={data} error={error} onClose={reset} onLogin={() => setShowModal(true)} />
      <form className="row g-3" onSubmit={handleOnSubmit}>
        ...
        <div className="col-12 mt-4">
          <button type="submit" className="btn btn-primary" disabled={disableSaveBtn || loading || !auth}>
...
const CustomAlert = ({
  ...
  !auth ? (
    ...
    <Alert data-testid="success-alert" msg="Event was successfully added." type="success" onClose={onClose} />
  ) : error ? (
    <Alert msg="Error occurred while saving event! Please try again later." type="danger" onClose={onClose} />
  ) : null;
