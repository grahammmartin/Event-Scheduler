...
const MyEvents: FC = () => {
  ...
  const selectRow: SelectRowProps<IEvent> = {
    mode: "checkbox",
    clickToSelect: true,
    onSelect: (row: IEvent, isSelected: boolean, rowIndex: number, e: SyntheticEvent) => {
  ...
  const updatedEvents = data?.getUserEvents.events.map(event => {
    const { id, title, start, end, isPrivate, description, url, createdBy, createdAt, updatedAt } = event;
  ...
  return (
    <TitledCard title="My Events">
      {loading || networkStatus === NetworkStatus.refetch ? (
        ...
        !updatedEvents?.length && <Alert msg={"No events were found"} type="warning" dismissible={false} />
      )}
      <ToolkitProvider bootstrap4 keyField="id" data={updatedEvents ?? []} columns={columns} search>
        {(props: ToolkitContextType) => (
          <div>
            <SearchBar {...props.searchProps} onSearch={setSearchTextHandler} searchText={searchText} delay={800} />
            <hr />
