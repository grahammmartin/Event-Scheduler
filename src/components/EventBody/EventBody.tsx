...
const EventBody: FC<Props> = ({ event, disableEdit, resetForm, onChangeValue, onValidate }) => {
  ...
  const handleValueChange = (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, prop: string) => {
    const { value } = event.target;

    const getValue = () => (prop === "isPrivate" ? (event.target as HTMLInputElement).checked : value);
  ...
  useEffect(() => {
    ...
    if (auth) {
      if (auth.userId === localEvent.createdById || localEvent.createdById === "") {
    ...
    <div className="row g-3">
      ...
      {errorMsg && !disableEdit && (
        <div className="col-12">
          <Alert msg={errorMsg} type="warning" onClose={() => setErrorMsg("")} />
        </div>
      )}
      <div className="col-12">
